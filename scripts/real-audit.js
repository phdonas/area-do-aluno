...
async function deepAudit() {
  console.log('--- Auditoria Real de Tabelas ---');
  ...
  const queries = {
    ...
    cols_aulas: "SELECT column_name, is_nullable, data_type FROM information_schema.columns WHERE table_name = 'aulas' AND is_nullable = 'NO'",
    cols_modulos: "SELECT column_name, is_nullable, data_type FROM information_schema.columns WHERE table_name = 'modulos' AND is_nullable = 'NO'"
  };

  for (const [name, query] of Object.entries(queries)) {
    const { data, error } = await supabase.rpc('query_sql', { sql_query: query });
    if (error) console.error(`Erro em ${name}:`, error.message);
    else console.log(`${name.toUpperCase()}:`, data);
  }
