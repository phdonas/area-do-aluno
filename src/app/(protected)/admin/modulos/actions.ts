...
export async function createModulo(formData: FormData) {
  ...
  // Tratamento se course_id for vazio ou 'null', deixamos null para ser um Módulo de Biblioteca
...
export async function reordenarAulaModuloPivot(moduloId: string, aulaId: string, novaOrdem: number) {
  const supabase = await createClient()
  // Check direct association first
  const { data: directMatch } = await supabase.from('aulas').select('id').eq('id', aulaId).eq('modulo_id', moduloId).single()
  if (directMatch) {
    await supabase.from('aulas').update({ ordem: novaOrdem }).eq('id', aulaId)
  } else {
    await supabase.from('modulos_aulas').update({ ordem: novaOrdem }).match({ modulo_id: moduloId, aula_id: aulaId })
  }
  revalidatePath(`/admin/modulos/${moduloId}`)
}
