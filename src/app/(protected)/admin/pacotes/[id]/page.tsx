...
import { updatePacote } from '../actions'

export default async function EditarPacotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const selectedCursosIds = (pacote.planos_cursos || []).map((cp: any) => cp.curso_id)
  const updatePacoteAction = updatePacote.bind(null, pacote.id)
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-text-primary">Editar Novo Produto / Pacote</h1>
      </div>

      <div className="bg-surface border border-border-custom p-6 md:p-8 rounded-2xl shadow-sm">
        <form action={updatePacoteAction} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <input 
                ...
                id="preco_mensal" 
                name="preco_mensal" 
                defaultValue={pacote.preco_mensal || ''}
                className="w-full bg-background border border-border-custom rounded-xl px-4 py-3 text-text-primary placeholder:text-text-muted focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <input 
                ...
                id="preco_anual" 
                name="preco_anual" 
                defaultValue={pacote.preco_anual || ''}
                className="w-full bg-background border border-border-custom rounded-xl px-4 py-3 text-text-primary placeholder:text-text-muted focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
              />
            </div>
          </div>
          ...
          <div className="p-4 bg-background border border-border-custom rounded-xl flex items-center justify-between gap-4 mt-6">
             ...
             <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" name="is_global" defaultChecked={pacote.is_global} className="sr-only peer" />
          ...
          <div className="space-y-3 mt-6">
             ...
             <select 
               name="cursos" 
               multiple 
               defaultValue={selectedCursosIds}
          ...
          <div className="pt-6 border-t border-border-custom flex justify-end gap-3 mt-8 items-center justify-between">
            <div className="flex items-center gap-2">
               <input type="checkbox" id="ativo" name="ativo" defaultChecked={pacote.ativo} className="w-4 h-4 text-primary bg-background border-border-custom rounded focus:ring-primary" />
