...
import { criarCupom, editarCupom } from '@/app/(protected)/admin/cupons/actions'
...
interface CouponModalProps {
  /** Quando informado, o modal opera em modo de edição em vez de criação */
  cupom?: Cupom
  /** Controle externo de abertura — usado pelo modo de edição (acionado via CouponActions) */
  open?: boolean
...
function cupomParaForm(cupom: Cupom) {
  return {
    codigo: cupom.codigo,
    tipo: cupom.tipo,
    valor: cupom.valor,
    data_inicio: cupom.validade_inicio ? cupom.validade_inicio.split('T')[0] : formInicial.data_inicio,
    data_fim: cupom.validade_fim ? cupom.validade_fim.split('T')[0] : '',
    limite_uso: cupom.limite_uso || 0,
    ativo: cupom.ativo,
    apenas_para_alunos: cupom.apenas_para_alunos ?? false
  }
}

export default function CouponModal({ cupom, open, onOpenChange }: CouponModalProps) {
  const isEdicao = !!cupom
  const [internalOpen, setInternalOpen] = useState(false)
  const [loading, setLoading] = useState(false)
  const [formData, setFormData] = useState(formInicial)

  const isOpen = isEdicao ? !!open : internalOpen
  const setIsOpen = (value: boolean) => {
    if (isEdicao) onOpenChange?.(value)
    else setInternalOpen(value)
  }

  // Preenche o formulário com os dados atuais do cupom ao abrir em modo edição
  useEffect(() => {
    if (isOpen && cupom) {
      setFormData(cupomParaForm(cupom))
    }
  }, [isOpen, cupom])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = isEdicao && cupom
      ? await editarCupom(cupom.id, payload)
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
               <div className="p-10 border-b border-border-custom flex items-center justify-between bg-black/[0.02]">
                  <div className="space-y-1">
                     <h3 className="font-ex-black text-text-primary uppercase tracking-widest text-xs flex items-center gap-3">
                        <TagIcon className="w-5 h-5 text-amber-500" /> {isEdicao ? 'Editar Cupom' : 'Engenharia de Oferta'}
                     </h3>
                     <p className="text-[10px] text-text-muted font-bold uppercase tracking-widest">{isEdicao ? `Ajustando ${cupom?.codigo}` : 'Inicie uma nova estratégia promocional'}</p>
                  </div>
               ...
               <form onSubmit={handleSubmit} className="p-10 space-y-10">
                  ...
                  >
                     {loading ? <LoaderIcon className="w-6 h-6 animate-spin" /> : isEdicao ? <><SaveIcon className="w-5 h-5" /> Salvar Alterações</> : <><SendIcon className="w-5 h-5" /> Projetar Campanha</>}
                  </button>
               </form>
            </motion.div>
          </div>
